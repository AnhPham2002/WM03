...
static uint8_t au8RxBuf[CELLULAR_BUFFER_SIZE];
...
/**
 * @brief Clear cellular module receive buffer.
 */
static void sv_cellular_flush_buffer(void);

/**
 * @brief Wait for one of the specified keywords from cellular module.
 *
 * @param[in] pKeywords     Keyword list.
 * @param[in] u8KeywordCount Number of keywords.
 * @param[in] u32Timeout    Timeout in milliseconds.
 *
 * @return true if a keyword is received within the timeout, otherwise false.
 */
static bool sv_cellular_wait_for_keywords(const char *const *pKeywords, uint8_t u8KeywordCount, uint32_t u32Timeout);
...
bool sv_cellular_off(void)
{
    ...
    drv_cellular_send(au8AtCommand, strlen((const char *)au8AtCommand));
...
bool sv_cellular_connect_tcp_socket(const uint8_t *pIp, uint8_t *pPort)
{
    ...
    drv_cellular_send((const uint8_t *)au8AtCommand, strlen(au8AtCommand));
...
bool sv_cellular_send_data(const uint8_t *pData, uint16_t u16Size)
{
    ...
    if (sv_cellular_wait_for_keywords(pKeywords1, ARRAY_SIZE(pKeywords1), AT_COMMAND_RESPONSE_TIMEOUT_NORMAL))
    {
        sv_cellular_flush_buffer();
        drv_cellular_send(pData, u16Size);
        drv_cellular_send((const uint8_t *)0x1A, 1); // Send CTRL+Z

        const char *pKeywords2[] = {"+CIPSEND:"}; // Syntax response: +CIPSEND: <link_num>,<reqSendLength>,<cnfSendLength>
        if (sv_cellular_wait_for_keywords(pKeywords2, ARRAY_SIZE(pKeywords2), AT_COMMAND_RESPONSE_TIMEOUT_NORMAL))
        {
...
bool sv_cellular_receive_data(uint8_t *pData, uint16_t *u16Size)
{
    ...
    {
        const char *pKeywords[] = {"\r\n+IPD"};
        if (!sv_cellular_wait_for_keywords(pKeywords, ARRAY_SIZE(pKeywords), HES_RESPONSE_TIMEOUT))
        {
            return false;
        }
...
bool sv_cellular_update_system_time(const char *pHost, float fTimezone)
{
    ...
    drv_cellular_send((const uint8_t *)au8AtCommand1, strlen(au8AtCommand1));
    ...
    if (sv_cellular_wait_for_keywords(pKeywords1, ARRAY_SIZE(pKeywords1), AT_COMMAND_RESPONSE_TIMEOUT_NORMAL))
    {
        ...
        drv_cellular_send((const uint8_t *)au8AtCommand2, strlen(au8AtCommand2));
...
static void sv_cellular_flush_buffer(void)
{
    memset(au8RxBuf, 0, CELLULAR_BUFFER_SIZE);
    u16RxBufSize = 0;
}
...
static bool sv_cellular_wait_for_keywords(const char *const *pKeywords, uint8_t u8KeywordCount, uint32_t u32Timeout)
{
    ...
    while (1)
    {
        if (drv_cellular_receive(au8RxBuf, &u16RxBufSize))
        {

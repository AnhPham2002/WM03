...
bool drv_protocol_pack(bool bEncrypt, uint64_t u64Serial, uint8_t u8Cmd, uint8_t u8Id, const uint8_t *pPayload, uint16_t u16PayloadLen, uint8_t *pFrame, uint16_t *u16FrameLen)
{
    ...
    else
    {
        ...
        u16CrcEncrpyt = sys_crc16(pCiphertext, sizeof(u16PayloadLen) + u16PayloadLen);
        memmove(pCiphertext + sizeof(u16PayloadLen) + u16PayloadLen, &u16CrcEncrpyt, sizeof(u16CrcEncrpyt)); // Copy crc encrypt to buffer

        u16PlaintextLen = sizeof(u16PayloadLen) + u16PayloadLen + sizeof(u16CrcEncrpyt);

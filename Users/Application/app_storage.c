...
{
    memcpy(pData, (const void *)BOOTLOADER_VERSION_ADDR, VERSION_SIZE);
}
...
{
    if ((u8PasswordIndex >= PASSWORD_COUNT) || (pPassword == NULL))
    {
        return false;
    }

    memcpy(sDevicePassword.au8Passwords[u8PasswordIndex], pPassword, PASSWORD_LENGTH);
...
bool app_storage_get_password(uint8_t u8PasswordIndex, uint8_t *pPassword)
{
    ...
    memcpy(pPassword, sDevicePassword.au8Passwords[u8PasswordIndex], PASSWORD_LENGTH);
    return true;
}

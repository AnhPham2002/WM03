...
void app_latch_execute(void)
{
    ...
    case LATCH_STEP_READ_PULSE_METER:
    {
        ...
        if ((!app_storage_check_meter_active(PULSE_METER_TYPE, u8Index)) || (!sv_pulse_meter_get_data(u8Index, &sPulseMeterData)))
        {
    ...
    case LATCH_STEP_READ_MODBUS_METER:
    {
        ...
        if ((!app_storage_check_meter_active(MODBUS_METER_TYPE, u8Index)) || (!sv_modbus_meter_get_data(u8Index, &sModbusMeterData)))
        {
    ...
    case LATCH_STEP_READ_PRESSURE_SENSOR:
    {
        ...
        if (((!app_storage_check_meter_active(PRESSURE_SENSOR_TYPE, u8Index))) || (!sv_pressure_sensor_get_data(u8Index, &sPressureSensorData)))
        {

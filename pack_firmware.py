...
OUTPUT_BIN_A = BASE_DIR / "build" / "Release" / "WM03_Firmware_Flash.bin"
OUTPUT_BIN_AB = BASE_DIR / "build" / "Release" / "WM03_Firmware_Ota.bin"
...
def main():
    ...
    # ============================================================
    # FILE 1: BOOTLOADER + METADATA A + APP A
    # ============================================================

    image_start_address_a = BOOTLOADER_START_ADDR
    image_end_address_a = SLOT_A_END_ADDR

    image_size_a = image_end_address_a - image_start_address_a
    image_a = bytearray([0xFF] * image_size_a)

    print("----- WM03_Firmware_Flash.bin -----")
    print(f"Start : 0x{image_start_address_a:08X}")
    print(f"End   : 0x{image_end_address_a:08X}")
    print(f"Size  : {image_size_a} bytes")
    print()
    ...
    OUTPUT_BIN_A.write_bytes(image_a)

    print()
    print(f"Output : {OUTPUT_BIN_A}")
    print(f"Size   : {len(image_a)} bytes")
    print()

    # ============================================================
    # FILE 2: METADATA A + APP A + METADATA B + APP B
    # ============================================================

    image_start_address_ab = SLOT_A_METADATA_ADDR
    image_end_address_ab = SLOT_B_END_ADDR

    image_size_ab = image_end_address_ab - image_start_address_ab
    image_ab = bytearray([0xFF] * image_size_ab)

    print("----- WM03_Firmware_Ota.bin -----")
    print(f"Start : 0x{image_start_address_ab:08X}")
    print(f"End   : 0x{image_end_address_ab:08X}")
    print(f"Size  : {image_size_ab} bytes")
    print()
    ...
    OUTPUT_BIN_AB.write_bytes(image_ab)

    print()
    print(f"Output : {OUTPUT_BIN_AB}")
    print(f"Size   : {len(image_ab)} bytes")
    print()
